			...
			if(isset($_SESSION['login_member'])){
				...
				<div style="float: right; padding: 25px;">
		 		...
		 		<div>
		 			<?php 
						$var = 0;
						$query = "SELECT * FROM `fine` WHERE username = '".$_SESSION['login_member']."' and status = 'not paid';";
		...
		<?php 
			if(isset($_SESSION['login_member'])){
				...
				if(isset($_POST['submit2'])){
					$sql="SELECT member.username,books.bid,name,authors,edition,approve,issue,issue_book.return FROM member inner join issue_book ON member.username = issue_book.username inner join books ON issue_book.bid=books.bid WHERE issue_book.approve = '$ret' AND issue_book.username ='$_SESSION[login_member]'  ORDER BY `issue_book`.`return` DESC";
					$res = mysqli_query($db,$sql);

				}
				else if(isset($_POST['submit3'])){
					$sql="SELECT member.username,books.bid,name,authors,edition,approve,issue,issue_book.return FROM member inner join issue_book ON member.username = issue_book.username inner join books ON issue_book.bid=books.bid WHERE issue_book.approve = '$exp' AND issue_book.username ='$_SESSION[login_member]' ORDER BY `issue_book`.`return` DESC";
					$res = mysqli_query($db,$sql);

				}
				else{
					$sql="SELECT member.username,books.bid,name,authors,edition,approve,issue,issue_book.return FROM member inner join issue_book ON member.username = issue_book.username inner join books ON issue_book.bid=books.bid WHERE issue_book.approve != '' and issue_book.approve != 'Yes'  AND issue_book.username ='$_SESSION[login_member]' ORDER BY `issue_book`.`return` DESC";
					$res = mysqli_query($db,$sql);
