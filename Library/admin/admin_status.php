	...
	<div class="srch">
 	...
 	<?php
 		if (isset($_POST['submit'])) {
 			$q=mysqli_query($db,"SELECT firstname ,lastname,username,email,phonenumber FROM `admin` where username like '%$_POST[search]%' and status='';");
 			if(mysqli_num_rows($q)==0){
 				echo "Sorry! No admin found with that username. Try searching again.";
 			}
 			else{
 			...
 			$res = mysqli_query($db,"SELECT firstname ,lastname,username,email,phonenumber FROM `admin` where status = '';");
